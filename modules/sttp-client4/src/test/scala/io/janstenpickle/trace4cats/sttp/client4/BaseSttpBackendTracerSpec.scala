...
abstract class BaseSttpBackendTracerSpec[F[_]: Async, G[_]: Sync: Trace, Ctx](
  ...
  liftBackend: Backend[F] => Backend[G]
  ...
  def test(runReq: (Backend[G], Request[Either[String, String]]) => G[Unit]): Assertion =
  ...
  def withBackend(app: HttpApp[F])(fa: Backend[G] => F[Assertion]): F[Assertion] =
